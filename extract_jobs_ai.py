...
Extract every distinct notification listed — this includes not just fresh vacancy/recruitment ads, but also exam results, merit lists, admit cards/hall tickets, and answer keys. A "marks and rank position published" or "result declared" announcement is just as much a notification to extract as a new job opening — don't skip it just because it isn't a vacancy.

Group by the notification's own reference number or identifying link, not by each post/subject/location named inside it. If a single link or line covers two or more notices with genuinely different reference numbers bundled together (e.g. "Advt. No. 048/2023 ... & Advt. No. 049/2023 ..."), split them into separate entries — each should stand on its own with its own title. But when ONE notification (one advertisement number, one PDF/link) lists many individual posts, subjects, or locations under that single recruitment drive (e.g. "Assistant Professor — Mathematics, Physics, Chemistry, Botany..." under one Advt. No.), that is ONE notification — extract it as a single entry and summarize the range of posts/subjects/locations in the description or vacancy_count, rather than creating a separate near-duplicate entry per post. When unsure, prefer one well-described entry over many entries that differ only by which post/subject/location is named.

For each one, include these fields when known:
